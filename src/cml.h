 ...
 *      and efficiency. Several computational-intense functions require/are enhanced by BLAS/LAPACK. The user is able 
 *      to define optional flags prior to including this library in source code to do things such as removing 
 *      BLAS/LAPACK as a dependency (while simultaneously removing several functions from API), or giving all functions 
 *      static storage. CML operates on type 'MATRIX' for all processes. 'MATRIX' is column major so the user can easily
 *      interface with the most popular linear-algebra-related API's. 'errno' is used for error handling extensively. 
 *      'errno' is set only in the presence of an error. The user is free to disregard 'errno' if an error occurred, but 
 *      the state of the resultant matrix is undefined. 
 *
 * USAGE:
